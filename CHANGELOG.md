...
### D4-TIA-15 v2.0.0 — evaluated-orbit profile

- Added an evaluated-covariant sonification profile downstream of the exact
  D4-TIA-COV algebra engine and D4-TIA-S3-EQUIV symmetry harness.
- Evaluated all 15 exact covariants across six canonical quotient
  representatives at the authored integer coefficient probe
  \((1,2,3,4,5,6,7)\) and Roberts anchor \((u,v)=(1,0)\).
- Required exact coefficient-action and variable-action evaluations to agree
  before sonification.
- Added a six-block symbolic event layout with the existing grading clock
  preserved within each orbit block.
- Mapped each generator's exact rational orbit-value classes to a small
  within-generator MIDI pitch contour without floating normalization.
- Preserved modular weight as base note, covariant order as channel,
  covariant degree as duration, and fixed velocity 64.
- Required all five order-zero covariants to keep identical exact values and
  identical MIDI note/channel/velocity/duration controls across all six orbit
  positions.
- Added deterministic JSON/CSV/MIDI artifacts, evaluation document, manifest,
  build command, verifier, tests, and documentation while retaining the root
  ban on rendered audio and canonical tempo/tuning.

### D4-TIA-S3-EQUIV v0.1.0 — exact symmetry harness
...
- Added the canonical harness fixture, verifier, tests, and documentation.
...
The freeze/tag/archive and Zenodo formalization steps for this line are complete.
Any future D4-TIA receiver remapping or semantic change must use a new version
rather than modifying the frozen v2.0.0 release.
