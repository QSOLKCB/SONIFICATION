...
const ALLOWED_EXTENSIONS = new Set(ALLOWED_ROOT_ARTIFACT_EXTENSIONS_V2);
const RENDERED_AUDIO_OPTION =
  /^--(?:wav|pcm|aiff|flac|mp3|ogg|audio|render-audio)(?:=|$)/i;

function parseArguments(argv = process.argv.slice(2)) {
  let output = resolve(OUTPUT_ROOT, "d4-tia-15-v2.0.0");
  let sawOutput = false;
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (RENDERED_AUDIO_OPTION.test(argument)) {
      throw new Error(
        `rendered-audio request is prohibited in the root D4-TIA v2 profile: ${argument}`,
      );
    }
...
function assertDedicatedOutputPath(output) {
  ...
  if (existsSync(output)) {
    ...
    if (stat.isSymbolicLink()) throw new Error("--output must not be a symbolic link");
    if (!stat.isDirectory()) {
...
const files = new Map([
  ...
  ["evaluations.json", bundle.evaluationBytes],
...
console.log(
  JSON.stringify(
    {
      status: "PASS",
      profile: "D4-TIA-15@2.0.0",
      output,
      fileCount: files.size,
      eventCount: bundle.eventDocument.eventCount,
      invariantProjection: bundle.eventDocument.invariantProjection,
